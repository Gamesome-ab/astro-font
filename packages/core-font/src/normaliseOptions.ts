...
export const _parsedType = (type: FontFamily["type"]) => {
	if (typeof type === "undefined") {
		...
		throw new Error(
			`@gamesome/core-font encounterd a font family with an invalid type (should be sans-serif, serif or mono). ${JSON.stringify(
				type
			)}`
		);
	}
	...
	if (typeof preloads === "undefined") {
		...
		return preloads.map((p) => {
			if (
				...
				throw new Error(
					`@gamesome/core-font encounterd an invalid list of preloads (string[]). ${JSON.stringify(
						preloads
					)}`
				);
			}
		...
		throw new Error(
			`@gamesome/core-font encounterd a font import with an invalid preload (should be array of Preload, undefined or boolean). ${JSON.stringify(
				preloads
			)}`
		);
	}
...
export const _parsedFontImport = (
	...
	if (typeof imp === "string") {
		...
		throw new Error(
			`@gamesome/core-font encounterd a font import without css import statement, or it could not be parsed: ${JSON.stringify(
				imp
			)}`
		);
	}
...
export const _parsedFallbackFont = (inp: FallbackFont | string) => {
	...
	if (!f.name) {
		throw new Error(
			`@gamesome/core-font encounterd a fallback font without a name. ${JSON.stringify(
				f
			)}`
		);
	}
	...
	if (typeof f.bold === "undefined") {
		...
		throw new Error(
			`@gamesome/core-font encounterd a fallback font with an invalid bold configuration (should be object with suffix and weight, undefined or false). ${JSON.stringify(
				f
			)}`
		);
	}
	...
	if (typeof f.scaling === "undefined") {
		...
		throw new Error(
			`@gamesome/core-font encounterd a fallback font with an invalid scaling (should be object, undefined or false). ${JSON.stringify(
				f
			)}`
		);
	}
...
export const _parsedFallbacks = (
	...
	if (typeof fallbacks === "undefined") {
		...
		throw new Error(
			`@gamesome/core-font encounterd a font family with an invalid fallbacks (should be (FallbackFont | string)[], undefined or false). ${JSON.stringify(
				fallbacks
			)}`
		);
	}
...
export const _parsedAppendFontFamilies = (
	...
	if (typeof appended === "undefined") {
		...
		throw new Error(
			`@gamesome/core-font encounterd a font family with an invalid appendFontFamilies (should be string, undefined or false). ${JSON.stringify(
				appended
			)}`
		);
	}
...
export const _parsedFamily = (f: Partial<FontFamily>, first: boolean) => {
	if (!(f.name && f.imports && f.imports.length)) {
		throw new Error(
			`@gamesome/core-font encounterd a font family without: name, type or imports. ${JSON.stringify(
				f
			)}`
		);
	}
...
export const parsedFamilies = (families: FontFamily[]) => {
	if (!families.length) {
		throw new Error(
			`@gamesome/core-font: you must specify at least one font family.`
		);
	}
